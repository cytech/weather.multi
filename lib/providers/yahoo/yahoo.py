...
class Weather():
    ...
    def get_weather(response, loc):
        ...
        set_property('Updated',
                     convert_datetime(data['observation']['observationTime']['timestamp'], 'datetime', 'timedate',
                                      None))
        ...
        set_property('Current.OutlookIcon', '%s.png' % str(
            data['observation']['conditionCode']))  # Kodi translates it to Current.ConditionIcon
        ...
        set_property('Current.DewPoint',
                     dewpoint(int(convert_temp(data['observation']['temperature']['now'], 'F', 'C')),
                              data['observation']['humidity']))
        ...
        set_property('Current.WindChill', convert_temp(
            windchill(data['observation']['temperature']['now'], data['observation']['windSpeed']), 'F') + TEMPUNIT)
        if 'F' in TEMPUNIT:
            ...
            set_property('Current.Pressure',
                         str(int(round((33.864 * data['observation']['barometricPressure'])))) + ' mbar')
        ...
        for count, item in enumerate(data['forecasts']['hourly']):
            set_property('Hourly.%i.Time' % (count + 1),
                         convert_datetime(item['observationTime']['timestamp'], 'datetime', 'time', None))
            set_property('Hourly.%i.LongDate' % (count + 1),
                         convert_datetime(item['observationTime']['timestamp'], 'datetime', 'monthday', 'long'))
            set_property('Hourly.%i.ShortDate' % (count + 1),
                         convert_datetime(item['observationTime']['timestamp'], 'datetime', 'monthday', 'short'))
            set_property('Hourly.%i.Temperature' % (count + 1),
            ...
            set_property('Hourly.%i.WindDirection' % (count + 1),
                         xbmc.getLocalizedString(WIND_DIR(item['windDirection'])))
            ...
            set_property('Hourly.%i.DewPoint' % (count + 1), convert_temp(
                dewpoint(int(convert_temp(item['temperature']['now'], 'F', 'C')), item['humidity']), 'C') + TEMPUNIT)
    ...
    def get_daily_weather(response):
        ...
        for count, item in enumerate(data['forecasts']['daily']):
            set_property('Daily.%i.ShortDay' % (count + 1),
                         convert_datetime(item['observationTime']['weekday'], 'day', 'weekday', 'short'))
            ...
            set_property('Daily.%i.HighTemperature' % (count + 1),
                         convert_temp(item['temperature']['high'], 'F') + TEMPUNIT)
            set_property('Daily.%i.LowTemperature' % (count + 1),
                         convert_temp(item['temperature']['low'], 'F') + TEMPUNIT)
            ...
            set_property('Daily.%i.DewPoint' % (count + 1), convert_temp(
                dewpoint(int(convert_temp(item['temperature']['low'], 'F', 'C')), item['humidity']), 'C') + TEMPUNIT)
